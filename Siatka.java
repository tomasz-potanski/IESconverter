public class Siatka extends SiatkaAbstrakcyjna {
    ...
    public double getIlluminanceFromField(int x, int y) {
        if (x < 0 ||
                y < 0 ||
                x >= liczba_prost_na_dlugosci ||
                y >= liczba_prost_na_szerokosci) {
            return 0.0;
        } else {
    ...
    public void setNatezenie(
            int wsp_dlugosci,
            int wsp_szerokosci,
            double nowe_natezenie) {

        if (wsp_dlugosci >= 0 &&
                wsp_dlugosci < liczba_prost_na_dlugosci &&
                wsp_szerokosci >= 0 &&
                wsp_szerokosci < liczba_prost_na_szerokosci) {
